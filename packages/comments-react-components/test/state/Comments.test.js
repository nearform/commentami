...
describe('Comments', () => {
  ...
  test('The default state should be correct', () => {
    ...
    expect(comments.defaultState).toEqual({ resources: {} })
  })
  ...
  describe('Adding a comment', () => {
    ...
    test('the size of the list of comments related to reference ref-1 should be 1', () => {
      expect(selectCommentsByReference(state, { resource: { id: 'res-1' }, reference: { id: 'ref-1' } }).length).toBe(1)
    })

    test('the comment should be added correctly', () => {
      expect(selectCommentsByReference(state, {
        resource: { id: 'res-1' },
        reference: { id: 'ref-1' }
      })[0]).toEqual(comment1)
    })
  ...
  describe('Removing a comment', () => {
    ...
    test('the size of the list of comments related to reference ref-2 should be 0', () => {
      expect(selectCommentsByReference(state, { resource: { id: 'res-1' }, reference: { id: 'ref-2' } }).length).toBe(0)
    })

    test('the size of the list of comments related to reference ref-3 should be 1', () => {
      expect(selectCommentsByReference(state, { resource: { id: 'res-1' }, reference: { id: 'ref-3' } }).length).toBe(1)
    })
  })
  ...
  describe('Get comments by reference', () => {
    beforeEach(() => {
      comments = new CommentsState(new CommentsInMemoryService(), getState, setState)
      comments.addComment({ resource: { id: 'res-1' }, reference: { id: 'ref-1' }, content: 'somecontent 1' })
      comments.addComment({ resource: { id: 'res-1' }, reference: { id: 'ref-1' }, content: 'somecontent 2' })
      comments.addComment({ resource: { id: 'res-1' }, reference: { id: 'ref-2' }, content: 'somecontent 3' })
      comments.addComment({ resource: { id: 'res-1' }, reference: { id: 'ref-2' }, content: 'somecontent 4' })
      comments.addComment({ resource: { id: 'res-1' }, reference: { id: 'ref-1' }, content: 'somecontent 5' })
    })

    test('the size should be 5', () => {
      expect(commentsCount(state, { resource: { id: 'res-1' }, reference: { id: 'ref-1' } })).toBe(3)
    })

    test('the size of the list of comments related to reference res-1 should be 3', () => {
      expect(selectCommentsByReference(state, { resource: { id: 'res-1' }, reference: { id: 'ref-1' } }).length).toBe(3)
    })

    test('the size of the list of comments related to reference res-2 should be 2', () => {
      expect(selectCommentsByReference(state, { resource: { id: 'res-1' }, reference: { id: 'ref-2' } }).length).toBe(2)
    })
    ...
    test('the comments should be returned correctly', () => {
      expect(selectCommentsByReference(state, {
        resource: { id: 'res-1' },
        reference: { id: 'ref-1' }
      })[0].content).toBe('somecontent 1')
      expect(selectCommentsByReference(state, {
        resource: { id: 'res-1' },
        reference: { id: 'ref-1' }
      })[1].content).toBe('somecontent 2')
      expect(selectCommentsByReference(state, {
        resource: { id: 'res-1' },
        reference: { id: 'ref-1' }
      })[2].content).toBe('somecontent 5')

      expect(selectCommentsByReference(state, {
        resource: { id: 'res-1' },
        reference: { id: 'ref-2' }
      })[0].content).toBe('somecontent 3')
      expect(selectCommentsByReference(state, {
        resource: { id: 'res-1' },
        reference: { id: 'ref-2' }
      })[1].content).toBe('somecontent 4')
    })
  })
})
