...
export class Sidebar extends React.Component {
  ...
  handleClose(ev) {
    ev.preventDefault()
    const payload = {
      resource: this.props.controller.resource,
      reference: this.props.controller.reference,
      ref: this.rootRef,
      scope: 'sidebar-close'
    }
  ...
  .add('Fetch Sample', () => (
    <CommentableController>
      <CommentableProvider resource="sample-table-section" service={commentsFetchService}>
        <div style={{ margin: '30px' }}>
          <Table data={sampleData} columns={['name', 'gender', 'email', 'balance']} blockComponent={Block} />
        </div>
        <CommentableSidebar />
      </CommentableProvider>
    </CommentableController>
